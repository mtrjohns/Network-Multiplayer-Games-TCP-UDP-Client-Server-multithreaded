...
int UDP::InitUDP()
{
	...
	if (iResult == 0)
	{
		...
		listenSocket = Socket("127.0.0.1", 27018, addressFamily, packetType, protocolType);

		clientSocket = Socket("127.0.0.1", 27019, addressFamily, packetType, protocolType);
...
int UDP::Send()
{
	const char* buf = "Hello UDP, From Client";
	cout << "Sending on UDP to Server = " << buf << endl;
	OutMemoryStream out;
	out = OutMemoryStream();
